...
- `SKAPXD_LAYER` is the runtime metadata key (`"skapxd:layer"`) used for optional introspection when `reflect-metadata` is available.
...
`@Dto` writes `SKAPXD_LAYER = "dto"` when `reflect-metadata` is loaded. Without `Reflect.defineMetadata`, it is a safe no-op because the linter cares about the decorator export name and import source.
...
`@UseCase` is composed with Nest's `@Injectable()` and sets `SKAPXD_LAYER = "use-case"`.
